...
export default async function CompanyHome({
  ...
  const allServices = getServices(company);
  
  // Custom data for each company
  const luxeData = {
    heroVideoUrl: "/interior.mp4",
    heroTitle: "Artis Luxe",
    heroDesc: "Elevating Spaces to Premium Destinations with bespoke interior designs and master craftsmanship.",
    stats: [
    ...
    features: [
      ...
      { title: "Bespoke Furniture: A Mark of Distinction", src: "https://images.unsplash.com/photo-1524758631624-e2822e304c36?q=80&w=400&auto=format&fit=crop" },
  ...
  const technicalData = {
    heroVideoUrl: "/technical.mp4",
    heroTitle: "Artis Technical",
    heroDesc: "Precision Industrial & Commercial Solutions for large-scale infrastructure and meticulous maintenance.",
    stats: [
  ...
  return (
    <div className="bg-background">
      ...
      {/* Services We Offer Section - Updated with Real Links */}
              ...
              {allServices.slice(0, 4).map((service) => (
                <li key={service.slug}>
                  <Link 
                    ...
                    className="flex items-center justify-between group cursor-pointer hover:text-primary transition-colors py-1"
                  >
                    <div className="flex items-center gap-4">
                      <span className="w-3 h-3 bg-primary block" />
                      <span className="text-lg font-medium">{service.title}</span>
                    </div>
                    <MoveRight className="w-5 h-5 text-primary opacity-0 group-hover:opacity-100 group-hover:translate-x-1 transition-all shrink-0" />
                  </Link>
                </li>
              ))}
              <li className="pt-4">
                <Link href={`/${company}/services`} className="text-primary font-bold flex items-center gap-2 hover:gap-4 transition-all">
                  View All Services <MoveRight className="size-5" />
                </Link>
              </li>
            </ul>
          </div>
          <div className="lg:w-3/4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 h-full">
              {data.featuredServices.map((service, i) => (
                <Link key={i} href={`/${company}/services/${service.slug}`} className="group relative h-64 md:h-full overflow-hidden block">
                  <img src={service.image} alt={service.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
                  <div className="absolute inset-0 bg-black/30 group-hover:bg-black/50 transition-colors" />
                  <div className="absolute inset-x-0 bottom-0 p-6 flex flex-col justify-end h-full">
                    <h3 className="text-white text-2xl font-semibold translate-y-2 group-hover:translate-y-0 transition-transform">{service.title}</h3>
                    <p className="text-white/70 text-sm mt-2 opacity-0 group-hover:opacity-100 transition-opacity duration-500 line-clamp-2">
                       {service.description}
                    </p>
                  </div>
                </Link>
